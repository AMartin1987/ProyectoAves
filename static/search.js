...
function initMap() {
  ...
  for (let i = 0; i < ubic_len; i++) {
    ...
    for (let m = 0; m < aves_len; m++) {
      // Chequea todas las aves posibles para esta ubicación
      if (ubic_dict[i].Id == aves_dict[m].Id_UBICACIONES) {
        for (let o = 0; o < espe_len; o++) {
          // Chequea todas las especies contra este ave
    ...
    for (let j = 0; j < aves_len; j++) {
      if (ubic_dict[i].Id == aves_dict[j].Id_UBICACIONES) {
        ...
        const info = new google.maps.InfoWindow({
          content:
            '<div style=line-height:2;text-align: left><span style="font-size:1.2em;font-weight: bold;">' +
            especie[j] +
            ...
            direccion +
            '<br><div id="datos_aves"> <a href="">Contacto y más información</a></div></div>',
          maxWidth: 200,
        });
        ...
        google.maps.event.addListener(info, "domready", function () {
          document
            .getElementById("datos_aves")
            .addEventListener("click", function () {
              var myModal = new bootstrap.Modal(
                document.getElementById("modal_aves")
              );
              myModal.show();
              ...
              document.getElementById("estsalud").innerHTML =
                aves_dict[j].EstSalud;
              ...
              document.getElementById("foto").innerHTML =
                '<img src="static/upload/' + aves_dict[j].Foto + '">';
            });
        });
    ...
    for (let k = 0; k < refug_len; k++) {
      if (ubic_dict[i].Id == refug_dict[k].Id_UBICACIONES) {
        ...
        const info2 = new google.maps.InfoWindow({
          content:
            '<div style=line-height:2;text-align: left><span style="font-size:1.2em;font-weight: bold;">Refugio para: </span><br>' +
            tiporef1 +
            tiporef2 +
            '<br><span style="text-decoration:underline;">Se reciben:</span> ' +
            especies +
            '<span>.</span><br><span style="text-decoration:underline;">Dirección:</span> ' +
            direccion +
            '<br><div id="datos_refugios"><a href="">Contacto y más información</a></div></div>',
        });
        especies = [];
        ...
        google.maps.event.addListener(info2, "domready", function () {
          document
            .getElementById("datos_refugios")
            .addEventListener("click", function () {
              var myModal = new bootstrap.Modal(
                document.getElementById("modal_refugios")
              );
              myModal.show();
              event.preventDefault();
              direccion = ubic_dict[i].Direccion.split(",");
