...
def search():
    ...
    if current_user.is_authenticated:
        nombre = current_user.name
    cur = get_db().cursor()
    ubicaciones = cur.execute(
        "SELECT Id, Latitud, Longitud, Direccion FROM UBICACIONES"
    ).fetchall()
    aves = cur.execute(
        "SELECT Id, Especie, Edad, Foto, EstSalud, Requer, Id_ESPECIES, Id_TELEFONOS, Id_TIPOREFUGIO, Id_UBICACIONES FROM AVES"
    ).fetchall()
    refugios = cur.execute(
        "SELECT Id, Id_ESPECIES1, Id_ESPECIES2, Id_ESPECIES3, Id_ESPECIES4, Id_UBICACIONES, Id_TELEFONOS, Id_TIPOREFUGIO1, Id_TIPOREFUGIO2 FROM REFUGIOS"
    ).fetchall()
    ...
    return render_template(
        ...
        ubic_dict=ubic_dict,
        aves_dict=aves_dict,
        refug_dict=refug_dict,
        telef_dict=telef_dict,
        espe_dict=espe_dict,
        tiporef_dict=tiporef_dict,
        nombre=nombre,
    )
...
def addbird():
    ...
    if form.validate_on_submit():
        ...
        if file_ext not in current_app.config["UPLOAD_EXTENSIONS"]:
            raise BadRequest(
                "Por favor suba un archivo de imagen válido (JPG, GIF o PNG). Tamaño máximo: 4 MB."
            )
        ...
        idEspecies = curs.execute(
            "SELECT Id FROM ESPECIES WHERE Categoria = (?)", (especie,)
        ).fetchone()
        ...
        for i in lista2:
            if i[0] == "('{0}',)".format(contacto):
                idEspecies = curs.execute(
                    "SELECT Id FROM TELEFONOS WHERE Telefono = (?)", (contacto,)
                ).fetchone()
                break
        if idTelefonos == "none":
        ...
        idTipoRefug = curs.execute(
            "SELECT Id FROM TIPOREFUGIO WHERE TipoRefug = (?)", (lugar,)
        ).fetchone()
        ...
        for i in lista4:
            if i[0] == "('{0}',)".format(localiz):
                idUbicaciones = curs.execute(
                    "SELECT Id FROM UBICACIONES WHERE Direccion = (?)", (localiz,)
                ).fetchone()
                break
        if idUbicaciones == "none":
            curs.execute(
                "INSERT INTO UBICACIONES (Latitud, Longitud, Direccion) VALUES (?, ?, ?)",
                (
                    loc_lat,
                    loc_long,
                    localiz,
        ...
        if tipoespecie[0] == "paloma":
            ...
            if len(tipoespecie) > 1:
                if tipoespecie[1] == "peqsil":
                    especie2 = (
                        2  # Si además de paloma tildó peqsil, eso además va a la col 2
                    )
                        ...
                        if tipoespecie[2] == "medsil":
                            ...
                            if len(tipoespecie) > 3:
                                if tipoespecie[3] == "corral":
                                    especie4 = (
                                        4  # Si además tildó corral, esto va en col 4
                                    )
                elif tipoespecie[1] == "medsil":
                    especie2 = (
                        3  # Si además de paloma tildó medsil, eso además va a la col 2
                    )
                    if len(tipoespecie) > 2:
                    ...
                    especie2 = (
                        4  # Si además de paloma tildó corral, eso además va a la col 2
                    )
...
def register():
    ...
    if form.validate_on_submit():
        ...
        curs = connection.cursor()
        curs.execute(
            "INSERT INTO USUARIOS (Nombre, Email, Hashed_password) VALUES(?, ?, ?)",
            (name, email, hash),
        )
        connection.commit()
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if form.email.data == Us.email and check_password_hash(
            ...
            return redirect(
                get_safe_redirect(next) or url_for("index")
            )  # safely redirect user to last accessed page
        else:
...
def profile():
    ...
    ubicaciones = cur.execute(
        "SELECT Id, Latitud, Longitud, Direccion \
                ...
                FROM REFUGIOS WHERE Id_USUARIOS = (?))",
        (user_id, user_id),
    ).fetchall()
    aves = cur.execute(
        "SELECT Id, Especie, Edad, Foto, EstSalud, Requer, \
         Id_ESPECIES, Id_TELEFONOS, Id_TIPOREFUGIO, Id_UBICACIONES FROM AVES \
         WHERE Id_USUARIOS = (?)",
        (user_id,),
    ).fetchall()
    refugios = cur.execute(
        "SELECT Id, Id_ESPECIES1, Id_ESPECIES2, Id_ESPECIES3, \
             Id_ESPECIES4, Id_UBICACIONES, Id_TELEFONOS, Id_TIPOREFUGIO1, \
             Id_TIPOREFUGIO2 FROM REFUGIOS WHERE Id_USUARIOS = (?)",
        (user_id,),
    ).fetchall()
    telefonos = cur.execute(
        "SELECT Id, Telefono FROM TELEFONOS WHERE Id IN \
              (SELECT Id_TELEFONOS FROM AVES WHERE Id_USUARIOS = (?) UNION \
              SELECT Id_TELEFONOS FROM REFUGIOS WHERE Id_USUARIOS = (?))",
        (user_id, user_id),
    ).fetchall()
    especies = cur.execute("SELECT Id, Categoria FROM ESPECIES").fetchall()
    tiporef = cur.execute("SELECT Id, TipoRefug FROM TIPOREFUGIO").fetchall()
    ...
    if refug_dict:
        for i in range(len(refug_dict)):
            if refug_dict[str(i)]["Id_ESPECIES1"] != "NULL":
                refug_dict[str(i)]["Id_ESPECIES1"] = translate(
                    refug_dict[str(i)]["Id_ESPECIES1"]
                )
            if refug_dict[str(i)]["Id_ESPECIES2"] != "NULL":
                refug_dict[str(i)]["Id_ESPECIES2"] = translate(
                    refug_dict[str(i)]["Id_ESPECIES2"]
                )
            if refug_dict[str(i)]["Id_ESPECIES3"] != "NULL":
                refug_dict[str(i)]["Id_ESPECIES3"] = translate(
                    refug_dict[str(i)]["Id_ESPECIES3"]
                )
            if refug_dict[str(i)]["Id_ESPECIES4"] != "NULL":
                refug_dict[str(i)]["Id_ESPECIES4"] = translate(
                    refug_dict[str(i)]["Id_ESPECIES4"]
                )
...
def delete_refugio():
    ...
    if delete_id_refugio:
        ...
        curs = connection.cursor()
        curs.execute(
            "DELETE FROM UBICACIONES WHERE Id IN (SELECT Id_UBICACIONES FROM REFUGIOS WHERE Id = (?))",
            (delete_id_refugio,),
        )
...
def delete_ave():
    ...
    if delete_id_ave:
        ...
        curs = connection.cursor()
        curs.execute(
            "DELETE FROM UBICACIONES WHERE Id IN (SELECT Id_UBICACIONES FROM AVES WHERE Id = (?))",
            (delete_id_ave,),
        )
...
def editbird():
    ...
    if request.method == "POST":
        ...
        telefonos = cur.execute(
            "SELECT Id, Telefono FROM TELEFONOS WHERE Id IN \
                (SELECT Id_TELEFONOS FROM AVES WHERE Id = (?))",
            (bird_id,),
        ).fetchall()
        get_db().commit()
...
def update_bird():
    ...
    if tipoespecie != "":
        if tipoespecie == "paloma":
            id_especie = curs.execute(
                "SELECT Id FROM ESPECIES WHERE Categoria = 'paloma'"
            ).fetchone()
            id_especie = id_especie[0]
            curs.execute(
                "UPDATE AVES SET Id_ESPECIES = (?) WHERE Id = (?)",
                (
                    id_especie,
                    bird_id,
                ),
        ...
        if tipoespecie == "peqsil":
            id_especie = curs.execute(
                "SELECT Id FROM ESPECIES WHERE Categoria = 'peqsil'"
            ).fetchone()
            id_especie = id_especie[0]
            curs.execute(
                "UPDATE AVES SET Id_ESPECIES = (?) WHERE Id = (?)",
        ...
        if tipoespecie == "medsil":
            id_especie = curs.execute(
                "SELECT Id FROM ESPECIES WHERE Categoria = 'medsil'"
            ).fetchone()
            id_especie = id_especie[0]
            curs.execute(
                "UPDATE AVES SET Id_ESPECIES = (?) WHERE Id = (?)",
        ...
        if tipoespecie == "corral":
            id_especie = curs.execute(
                "SELECT Id FROM ESPECIES WHERE Categoria = 'corral'"
            ).fetchone()
            id_especie = id_especie[0]
            curs.execute(
                "UPDATE AVES SET Id_ESPECIES = (?) WHERE Id = (?)",
    ...
    if especie != "":
        curs.execute(
            "UPDATE AVES SET Especie = (?) WHERE Id = (?)",
            (
                especie,
                bird_id,
            ),
    ...
    if estsalud != "":
        curs.execute(
            "UPDATE AVES SET EstSalud = (?) WHERE Id = (?)",
            (
                estsalud,
                bird_id,
            ),
    ...
    if requer != "":
        curs.execute(
            "UPDATE AVES SET Requer = (?) WHERE Id = (?)",
            (
                requer,
                bird_id,
            ),
    ...
    if tiporef != "":
        if tiporef == "hogar":
            id_tiporef = curs.execute(
                "SELECT Id FROM TIPOREFUGIO WHERE TipoRefug = 'hogar'"
            ).fetchone()
            id_tiporef = id_tiporef[0]
            curs.execute(
                "UPDATE AVES SET Id_TIPOREFUGIO = (?) WHERE Id = (?)",
                (
                    id_tiporef,
                    bird_id,
                ),
            ...
            id_tiporef = curs.execute(
                "SELECT Id FROM TIPOREFUGIO WHERE TipoRefug = 'transito'"
            ).fetchone()
            id_tiporef = id_tiporef[0]
            curs.execute(
                "UPDATE AVES SET Id_TIPOREFUGIO = (?) WHERE Id = (?)",
            ...
            connection.commit()
    if ubicacion != "":
        id_ubicaciones = curs.execute(
            "SELECT Id_UBICACIONES FROM AVES WHERE Id = (?)", (bird_id,)
        ).fetchone()
        id_ubicaciones = id_ubicaciones[0]
        curs.execute(
            "UPDATE UBICACIONES SET Direccion = (?) WHERE Id = (?)",
            (
                ubicacion,
                id_ubicaciones,
            ),
        ...
        connection.commit()
    if telef != "":
        id_telefonos = curs.execute(
            "SELECT Id_TELEFONOS FROM AVES WHERE Id = (?)", (bird_id,)
        ).fetchone()
...
def editplace():
    ...
    if request.method == "POST":
        ...
        refugios = cur.execute(
            "SELECT Id, Id_USUARIOS, Id_UBICACIONES, Id_ESPECIES1, Id_ESPECIES2, Id_ESPECIES3, Id_ESPECIES4, Id_TELEFONOS, Id_TIPOREFUGIO1, Id_TIPOREFUGIO2 \
               FROM REFUGIOS WHERE Id = (?)",
            (refugio_id,),
        ).fetchall()
        telefonos = cur.execute(
            "SELECT Id, Telefono FROM TELEFONOS WHERE Id IN \
                (SELECT Id_TELEFONOS FROM REFUGIOS WHERE Id = (?))",
            (refugio_id,),
        ).fetchall()
        get_db().commit()
...
def update_place():
    ...
    curs.execute(
        "UPDATE REFUGIOS SET Id_TIPOREFUGIO1 = (?), Id_TIPOREFUGIO2 = (?) WHERE Id = (?)",
        (
            tipoRef1,
            tipoRef2,
            place_id,
    ...
    curs.execute(
        "UPDATE REFUGIOS SET Id_ESPECIES1 = (?), Id_ESPECIES2 = (?), Id_ESPECIES3 = (?), Id_ESPECIES4 = (?) WHERE Id = (?)",
        (
            especie1,
            especie2,
            especie3,
    ...
    if ubicacion != "":
        id_ubicaciones = curs.execute(
            "SELECT Id_UBICACIONES FROM REFUGIOS WHERE Id = (?)", (place_id,)
        ).fetchone()
        id_ubicaciones = id_ubicaciones[0]
        curs.execute(
            "UPDATE UBICACIONES SET Direccion = (?) WHERE Id = (?)",
    ...
    if telef != "":
        id_telefonos = curs.execute(
            "SELECT Id_TELEFONOS FROM REFUGIOS WHERE Id = (?)", (place_id,)
        ).fetchone()
        id_telefonos = id_telefonos[0]
        curs.execute(
            "UPDATE TELEFONOS SET Telefono = (?) WHERE Id = (?)",
